...
import gc
import argparse
from functions_new import preprocessing_no_patching, create_boxes_cor, create_boxes_sag, preprocessing_coronal_sagittal
...
parser.add_argument('-batch_size', '--batch_size', type=int, 
                    help='batch size used for dataloader', default = 12 )
...
parser.add_argument('-N_augmentations', '--N_augmentations', type=int, 
                    help='what is the factor of increased volumes because of data augmentations', default = 4 )
args = parser.parse_args()


torch.manual_seed(0)

data = np.load(args.inputdir + "/classification_data.npz",allow_pickle=True)
X_train = data["X_train"]
X_train_new = np.moveaxis(X_train, 3,1)
X_test = data["X_test"]
X_new = np.concatenate((X_train, X_test),axis=0)

X_test_new = torch.tensor(np.moveaxis(X_test, 3,1))
data_patches = np.load(args.inputdir + "/Labels_patches.npz", allow_pickle=True)
Y_patch = data_patches["Y_patch"] #all patches of Labels available 10854
Y_patch_new = np.moveaxis(Y_patch, 3,1)
Y_patch_test_new = torch.tensor(Y_patch_new[-1416:])
Y_patch_train_new = torch.tensor(Y_patch_new[:-1416])
del Y_patch_new
gc.collect()


indices = np.load(args.inputdir + "/indices.npz")
...
# Split x_new into volumes based on indices
volumes = (split_volumes(X_new, indices))
volumes_gt = split_volumes(Y_patch, indices)

N_augmentations = args.N_augmentations
N_training_data = args.N_training_data
def augment_image(mri_volume: torch.Tensor, gt, indi, num_augmentations = N_augmentations):
    """
    Applies augmentation to a 3D MRI volume.

    Parameters:
    mri_volume (torch.Tensor): 3D tensor of shape (number_of_slices, 64, 64)
    gt: Ground truth label
    indi: Additional individual identifier
    num_augmentations (int): Number of augmentations to perform

    Returns:
    list of torch.Tensor: List of augmented 3D tensors
    list: Augmented ground truth labels
    list: Augmented individual identifiers
    """
    augmented_volumes = []
    augmented_gt = [gt] * num_augmentations
    augmented_indi = [indi] * num_augmentations
    ...
    for _ in range(num_augmentations):
        ...
        for slice_idx in range(mri_volume.shape[0]):
            slice_2d = mri_volume[slice_idx]

            # Convert the tensor slice to a PIL image
            pil_image = to_pil(slice_2d.unsqueeze(0))

            # Apply random contrast enhancement
            contrast_factor = random.uniform(0.5, 1.5)
            enhancer = ImageEnhance.Contrast(pil_image)
            enhanced_image = enhancer.enhance(contrast_factor)
            enhanced_image_tensor = to_tensor(enhanced_image).squeeze(0)

            # Apply random noise
            noise = torch.randn(slice_2d.size()) * 0.1
            noisy_image_tensor = slice_2d + noise
            noisy_image_tensor = torch.clamp(noisy_image_tensor, 0, 1)

            # Choose randomly between contrast enhancement, noisy image, and the original image
            augmented_slice = random.choice([enhanced_image_tensor, noisy_image_tensor, slice_2d])
            ## we also normalize each slice
            augmented_slice = augmented_slice - torch.mean(augmented_slice)
            augmented_slice = augmented_slice/torch.std(augmented_slice)
            augmented_slices.append(augmented_slice)

        augmented_volume = torch.stack(augmented_slices)
        augmented_volumes.append(augmented_volume)
    return (augmented_volumes), augmented_gt, augmented_indi
...
for i in range(len(volumes)):
    image_tensor = torch.tensor(volumes[i][:,:,:,0]).float().to(device)  # Add channel dimension and move to GPU
    label_tensor = torch.tensor(volumes_gt[i][:,:,:,0]).float().to(device) 
    augmented_images, augmented_gt, indices = (augment_image(image_tensor, label_tensor, indices))
    Augmented_train.append(augmented_images)
    Augmented_Gt_train.append(augmented_gt)
    Indices.append(indices)


def create_sag_cor_patches(X_new, Y_patch, indices, N_augmentations, N_training_data, N_total):
    ''' generates the training patches for coronal and sagittal training, which are then fed in corr. dataloaders'''
    X_train_sag = []
    Y_train_sag = []
    X_val_sag = []
    Y_val_sag = []
    ...
    thalamus_train = list(np.ones(int(N_augmentations*N_training_data))*50)
    thalamus_val = list(np.ones(int(N_total*N_augmentations-N_augmentations*N_training_data)*50))
    sag = preprocessing_coronal_sagittal(X_new,Y_patch,indices,thalamus_train,thalamus_val, N_augmentations, N_training_data, 'sag')
    cor = preprocessing_coronal_sagittal(X_new,Y_patch,indices,thalamus_train,thalamus_val, N_augmentations, N_training_data, 'cor')
    ...
    data_cor = torch.cat([torch.tensor(X_train_cor[0]), torch.tensor(Y_train_cor[0])], axis=1)
    data_cor_val = torch.cat([torch.tensor(X_val_cor[0]), torch.tensor(Y_val_cor[0])], axis=1)
    data_sag = torch.cat([torch.tensor(X_train_sag[0]), torch.tensor(Y_train_sag[0])], axis=1)
    data_sag_val = torch.cat([torch.tensor(X_val_sag[0]), torch.tensor(Y_val_sag[0])], axis=1)

    return data_cor, data_cor_val, data_sag, data_sag_val



T1 = [tensor for sublist in Augmented_train for tensor in sublist] #has shappe N*4,M, 64,64
GT = [tensor for sublist in Augmented_Gt_train for tensor in sublist]
Indi = [tensor for sublist in Indices for tensor in sublist]
N_total = 98


"""follwoing gives a tensor of shape 4 times number of training data, 64, 64, 1"""
T1 = (torch.cat(T1,0).unsqueeze(-1))
GT = (torch.cat(GT,0).unsqueeze(-1))
Indices = (torch.cat(torch.tensor(Indi),0))
print(Indices)
print(T1.shape, GT.shape, Indi.shape)

cor, cor_val, sag, sag_val = create_sag_cor_patches(T1, GT, Indi, args.N_augmentations, args.N_training_data, N_total)
print(cor.shape)

dataset = torch.utils.data.TensorDataset(T1[:int(np.sum(Indices[:N_augmentations*N_training_data]))], GT[:int(np.sum(Indices[:N_augmentations*N_training_data]))], Indi[:int(np.sum(Indices[:N_augmentations*N_training_data]))])
Data_loader = DataLoader(dataset, batch_size=args.batch_size, shuffle = True)


dataset_val = torch.utils.data.TensorDataset(T1[int(np.sum(Indices[N_augmentations*N_training_data])):], GT[int(np.sum(Indices[:N_augmentations*N_training_data])):], Indi[int(np.sum(Indices[:N_augmentations*N_training_data])):])
Data_loader_val = DataLoader(dataset_val, batch_size=args.batch_size, shuffle = True)
...
class Segmentation_of_PLIC:
    def __init__(
        ...
        inputs: str = "T1",
        sliceview: str= "axial",
        N_augmentations: int= args.N_augmentations,
        N_training_data: int = args.N_training_data,
        N_total: int = 98
    ):
        self.mean_FP=[]
        self.mean_FN=[]
