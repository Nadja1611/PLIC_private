...
import gc
import argparse
from functions import preprocessing_sagittal, preprocessing_coronal
...
def augment_image(image: torch.Tensor, gt, num_augmentations: int = 4):
    augmented_images = []
    augmented_gt = [gt] * num_augmentations
    
    # Convert the tensor to PIL image for contrast enhancement
    ...
    for _ in range(num_augmentations):
        ...
        enhanced_image_tensor = to_tensor(enhanced_image)

        # Apply random noise
        noise = torch.randn(image.size()) * 0.1
        noisy_image_tensor = image + noise
        ...
        augmented_image = random.choice([enhanced_image_tensor, noisy_image_tensor, image])
        
        augmented_images.append(augmented_image)
        augmented_gt.append(gt)

    return torch.stack(augmented_images), augmented_gt
...
for i in range(len(X_train_new)):
    image_tensor = (X_train_new[i]).float().unsqueeze(0).to(device)  # Add channel dimension and move to GPU
    label_tensor = (Y_patch_train_new[i]).float().unsqueeze(0).to(device) 
    augmented_images, augmented_gt = augment_image(image_tensor, label_tensor)
    Augmented_train.append(augmented_images)
    Augmented_Gt_train.append(augmented_gt)
for i in range(len(X_test_new)):
    image_val_tensor = (X_test_new[i]).float().unsqueeze(0).to(device) 
    label_val_tensor =(Y_patch_test_new[i]).float().unsqueeze(0).to(device)
    augmented_images_val, augmented_gt_val= augment_image(image_val_tensor, label_val_tensor)
    Augmented_test.append(augmented_images_val)
    Augmented_Gt_test.append(augmented_gt_val)
     
    

#### We now scale the images such that lie in range [0,1]
Images = torch.zeros_like(Augmented_train)
for i in range(len(Augmented_train)):
    Images[i] = Augmented_train[i]-np.min(Augmented_train[i])
    Images[i] = Images[i]/np.max(Images[i]+1e-10)
Images_Val = torch.zeros_like(Augmented_test)
for i in range(len(Augmented_test)):
    Images_Val[i] = Augmented_test[i]-np.min(Augmented_test[i])
    Images_Val[i] = Images_Val[i]/np.max(Images_Val[i]+1e-10)


data = torch.cat([(Images),(Augmented_Gt_train)],axis=1)
data_val = torch.cat([(Images_Val), (Augmented_Gt_test)], axis=1)
...
class Segmentation_of_PLIC:
    ...
    def create_sag_cor_patches_test(self, X_new,  indices):
        ''' generates the test patches for coronal and sagittal training'''
        X_train_sag = []
        Y_train_sag = []
        X_val_sag = []
        Y_val_sag = []
